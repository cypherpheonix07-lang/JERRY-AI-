...
import { runHermes } from "@/lib/integrations/hermesClient";
...
export const HermesPanel = () => {
  ...
  const run = async () => {
    setBusy(true); setOut('');
    const r = await runHermes(args ? args.split(' ') : []);
    setOut(JSON.stringify(r, null, 2)); setBusy(false);
  };

  return (
    <HudPanel title="Hermes Agent" icon={null}>
      <div className="space-y-2">
        ...
        <button onClick={run} disabled={busy} className="p-2 bg-primary/20 border border-primary rounded text-xs">{busy ? 'RUNNING...' : 'Run Hermes'}</button>
