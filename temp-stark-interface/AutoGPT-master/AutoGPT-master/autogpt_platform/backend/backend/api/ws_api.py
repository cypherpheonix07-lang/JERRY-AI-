...
async def handle_subscribe(
    ...
    elif message.method == WSMethod.SUBSCRIBE_HARDWARE:
        channel_key = await connection_manager.subscribe_hardware(
            user_id=user_id,
            websocket=websocket,
        )
...
async def handle_unsubscribe(
    ...
    # Handle both hardware and graph execution unsubscriptions
    try:
        # Check if we're unsubscribing from hardware
        if isinstance(message.data, dict) and message.data.get("type") == "hardware":
            channel_key = await connection_manager.unsubscribe_hardware(
                user_id=user_id,
                websocket=websocket,
            )
        else:
            # Default to graph execution unsubscribe
        ...
        # Fallback to graph execution unsubscribe if hardware check fails
        unsub_req = WSSubscribeGraphExecutionRequest.model_validate(message.data)
        channel_key = await connection_manager.unsubscribe_graph_exec(
            user_id=user_id,
            graph_exec_id=unsub_req.graph_exec_id,
...
_MSG_HANDLERS: dict[WSMethod, WSMessageHandler] = {
    ...
    WSMethod.SUBSCRIBE_HARDWARE: handle_subscribe,
