...
_EVENT_TYPE_TO_METHOD_MAP: dict[str, WSMethod] = {
    "graph_exec_update": WSMethod.GRAPH_EXECUTION_EVENT,
    "node_exec_update": WSMethod.NODE_EXECUTION_EVENT,
    "hardware_data_event": WSMethod.HARDWARE_DATA_EVENT,
}
...
class ConnectionManager:
    ...
    async def subscribe_hardware(
        self, *, user_id: str, websocket: WebSocket
    ) -> str:
        channel_key = f"hardware:{user_id}"
        full_channel = event_bus_channel(f"hardware:{user_id}")
        await self._open_subscription(websocket, channel_key, full_channel)
        return channel_key

    async def unsubscribe_hardware(
        self, *, user_id: str, websocket: WebSocket
    ) -> str | None:
        channel_key = f"hardware:{user_id}"
        return await self._close_subscription(websocket, channel_key)
